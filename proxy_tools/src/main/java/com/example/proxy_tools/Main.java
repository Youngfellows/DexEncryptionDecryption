...
public class Main {
    ...
    private static String dex = "D:/Android/sdk/build-tools/26.0.1/";
...
//        AES.init(AES.DEFAULT_PWD);
    ...
    private static void zipalign() throws IOException, InterruptedException {
        ...
        String text = "cmd /c" + dex + "zipalign -v -p 4 " + unSignedApk.getAbsolutePath() + " " + alignedApk.getAbsolutePath();
        Process process = Runtime.getRuntime().exec(text);
        process.waitFor();
...
//        System.out.println(process.waitFor() == 0 ? "zipalign成功" : "zipalign失败");
    ...
    public static void jksToApk() throws IOException, InterruptedException {
        ...
        String text = "cmd /c" + dex + "apksigner sign --ks " + jks.getAbsolutePath()
                + " --ks-key-alias yangkun --ks-pass pass:123123 --key-pass pass:123123 --out "
                + signedApk.getAbsolutePath() + " " + alignedApk.getAbsolutePath();
        Process process =  Runtime.getRuntime().exec(text);
        process.waitFor();
